	...
	jobs.reverse().map((v, i) => {
		html += `
			<tr>
				...
				<td>${v.updatedDate}</td>
				<td>
					<button class="btn btn-danger btn-sm" onclick="deleteJob(${v.jobID})">Xóa</button>
					<button class="btn btn-warning btn-sm" onclick="renderUpdate(${v.jobID})">Chỉnh sửa</button>
				</td>
			</tr>`;
	});
	row.innerHTML = html;
...
const renderUpdate = async (id) => {
	...
	document.getElementById('mota').value = data.data.jobDesc;
